...
def _run_server() -> None:
    import uvicorn
    from agentic_rag.src.config import API_HOST, API_PORT

    uvicorn.run("src.api:app", host=API_HOST, port=API_PORT, reload=True)


def _run_cli(article_code: str) -> None:
    from agentic_rag.src.config import CHROMA_PERSIST_DIR
    from agentic_rag.src.vectorstore_setup import initialize_vectorstore
    from agentic_rag.src.data_loader import load_datasets, get_train_test_split
    from agentic_rag.src.graph import classify_article
